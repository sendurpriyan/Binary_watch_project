...
#define LED_PIN 11     // Pin connected to the data pin of the LEDs
#define NUM_LEDS 16    // Total number of LEDs
#define BRIGHTNESS 25  // Total number of LEDs

// CRGB leds[NUM_LEDS];
AsyncWebServer server(80);
Preferences preferences;

const char *default_ssid = "BW_Hotspot";
...
void setup() {
  ...
  ssid = preferences.getString("ssid", default_ssid);  // Use default SSID if none is saved
  password = preferences.getString("password", "");    // Get stored password or empty string

  // Fallback to default password if none is found in preferences
  if (password == "") {
  ...
  Serial.print("Starting WiFi: ");
  delay(1000);
  WiFi.softAP(ssid.c_str(), password.c_str());  // Set up hotspot with either saved or default password

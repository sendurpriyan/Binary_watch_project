...
void setup() {
  ...
  Serial.printf("Restored time: %02d:%02d\n", rtcData.timeinfo.tm_hour, rtcData.timeinfo.tm_min);
  } else {
    // If no saved time, sync with NTP
    Serial.println("No saved time, syncing with NTP...");
  ...
  if (!hasBeenReprogrammed) {
    Serial.println("Resetting password to default.");
...
<body>
  <div class="container">
    ...
    <div class="password-reset">
      <input type="text" id="new_password" name="new_password" placeholder="Enter Current Password" required>
	<button type="submit">Change Password</button>
    </div>
</form>
...
server.on("/change_password", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (request->hasParam("new_password")) {
      ...
      // Debugging to ensure password is received
      ...
      // Debugging to confirm the password has been changed
...
void loop() {
  ...
  Serial.print(rtcData.red);
  Serial.print(",");
  Serial.print(rtcData.green);
  Serial.print(",");
  Serial.println(rtcData.blue);
...
void setTimeFromWeb(String newTime, String time_format) {
  int hour = newTime.substring(0, 2).toInt();
  int minute = newTime.substring(3, 5).toInt();
  int t_for = time_format.substring(0,2).toInt();
  ...
  struct tm t;
  time(&now);
  localtime_r(&now, &rtcData.timeinfo);
  
  rtcData.timeinfo.tm_hour = hour;
  rtcData.timeinfo.tm_min = minute;
  rtcData.timeinfo.tm_sec = 0;
  rtcData.t_format = t_for;

    // Save time to preferences
  preferences.putInt("savedHour", rtcData.timeinfo.tm_hour);
  preferences.putInt("savedMinute", rtcData.timeinfo.tm_min);
  preferences.putUInt("t_for", t_for);

  now = mktime(&rtcData.timeinfo);
  timeval tv = { now, 0 };
  settimeofday(&tv, nullptr);
}
